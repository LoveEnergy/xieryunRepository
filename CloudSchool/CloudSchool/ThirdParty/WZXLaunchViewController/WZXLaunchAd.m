...
- (UIButton *)skipBtn{
    ...
    if(_skipBtn == nil){
        ...
        _skipBtn = [[UIButton alloc]initWithFrame:CGRectMake([[UIScreen mainScreen] bounds].size.width-70, NAVI_HEIGHT, 60, 30)];
...
+ (instancetype)initImageWithframe:(CGRect )frame imageURL:(NSURL *)imageURL timeSecond:(NSInteger )timeSecond hideSkip:(BOOL)hideSkip LaunchAdCallback:(LaunchAdCallback)LaunchAdCallback ImageClick:(LaunchAdClick)ImageClick endPlays:(EndPlays)endPlays{
    ...
    [launchAd.adImageView sd_setImageWithURL:imageURL completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
//        SDImageCache * temp = [[SDImageCache alloc] init];
//        [temp clearMemory];
